  ...
  while (std::getline(std::cin, command)) {
      ...
      if (command.substr(0, 3) == add && command[3] == ' ' || command.substr(0, 3) == del && command[3] == ' ' || command.substr(0, 4) == find && command[4] == ' ' || command.substr(0, 5) == print)
      {
          ...
          else
          {
              ...
              while (isdigit(command[index]) || command[index] == '-' || command[index] == '+') //Looping until an illegal symbol will be entered
              {
                  //Check for the end of date expression or the end of a command
